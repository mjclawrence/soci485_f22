...
cces_2008 <- cces |> 
  ...
  summarise(dem_proportion = wtd.mean(voted_pres_08 == "Barack Obama",
                                      weights = weight_08),
            rep_proportion = wtd.mean(voted_pres_08 == "John McCain",
                                      weights = weight_08),
            other_proportion = wtd.mean(str_detect(voted_pres_08, "Other"),
                                        weights = weight_08)) |> 
  mutate(year = 2008)
```
...
cces_2012 <- cces |> 
  ...
  summarise(dem_proportion = wtd.mean(voted_pres_12 == "Barack Obama",
            ...
            rep_proportion = wtd.mean(voted_pres_12 == "Mitt Romney",
                                      weights = weight_cumulative),
            other_proportion = wtd.mean(str_detect(voted_pres_12, "Other"),
                              weights = weight_cumulative)) |> 
  mutate(year = 2012)
```
...
cces_2016 <- cces |> 
  ...
  summarise(dem_proportion = wtd.mean(voted_pres_16 == "Hilary Clinton",
                                      weights = weight_cumulative),
            rep_proportion = wtd.mean(voted_pres_16 == "Donald Trump",
                                      weights = weight_cumulative),
            other_proportion = wtd.mean(str_detect(voted_pres_16, "Other"),
                              weights = weight_cumulative)) |> 
  mutate(year = 2016)
```
...
cces_2020 <- cces |> 
  ...
  summarise(dem_proportion = wtd.mean(voted_pres_20 == "Joe Biden",
                                      weights = weight_cumulative),
            rep_proportion = wtd.mean(voted_pres_20 == "Donald Trump",
                                      weights = weight_cumulative),
            other_proportion = wtd.mean(str_detect(voted_pres_20, "Other"),
                              weights = weight_cumulative)) |> 
  mutate(year = 2020)
```
...
cces_08_20_all |> 
  ...
  theme(legend.position = "bottom") +scale_color_manual(values = c("blue", "darkgreen", "red"))
```
...
```{r}
write.csv(cces_08_20, "../../data/cces_08_20_threeparty_states.csv", row.names = FALSE)
```

```{r}
write.csv(cces_08_20_all, "../../data/cces_08_20_threeparty_all.csv", row.names = FALSE)
```
