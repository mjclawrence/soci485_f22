...
#write.csv(controls_merge, "data/nonwhite_coefficients.csv", row.names = FALSE)
  ...
  output$coefficient_plot <- renderPlotly({
    plot1 <- controls_merge |> 
      ...
      ggplot(aes(y = reorder_within(variable_name, nonwhite_coefficient_control, xsite), 
                 ...
                 text = paste0("In ", xsite, ", an increase of 1 standard deviation in the percent of nonwhite residents in a zipcode <br>is associated with an increase in the eviction percent of ", round(nonwhite_coefficient_nocontrol, 3),
                               ".<br>After controlling for ", variable_name, " that association changes to ", round(nonwhite_coefficient_control, 3), "."))) + 
      scale_y_reordered() +
    ...
    ggplotly(plot1,
             tooltip = "text") |> 
      layout(hoverlabel = list(align = "center"))
  })
